...
def get_devices_with_verifications_for_destination_by_date_range(destination_id: int, start_date: str, end_date: str):
    ...
    """
    with DatabaseConnection() as conn:
        # --- INIZIO QUERY AVANZATA ---
        # Questa query utilizza una subquery (CTE) per numerare le verifiche di ogni
        # dispositivo in ordine di data decrescente. Successivamente, la query principale
        # esegue un LEFT JOIN solo con la verifica numero 1 (la più recente).
        query = """
            ...
            ORDER BY 
                d.description;
        """
        # I parametri devono corrispondere ai '?' nella query
...
def get_devices_with_last_verification():
    ...
    """
    # --- CORREZIONE: 'date' è stato sostituito con 'verification_date' ---
    query = """
    SELECT
        d.*,
    ...
    FROM
        devices d
    LEFT JOIN (
        -- Sottoquery per trovare l'ultima verifica per ogni dispositivo
        SELECT
            device_id,
            MAX(verification_date) AS max_date  -- Corretto qui
        FROM
            verifications
        GROUP BY
            device_id
    ) AS latest_ver ON d.id = latest_ver.device_id
    LEFT JOIN verifications v ON v.device_id = latest_ver.device_id AND v.verification_date = latest_ver.max_date -- Corretto qui
    WHERE 
        d.is_deleted = 0
    ORDER BY
        d.id DESC;
