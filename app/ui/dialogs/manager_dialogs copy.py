...
from PySide6.QtWidgets import (QApplication, QDialog, QVBoxLayout, QHBoxLayout, QGroupBox, 
    ...
    QMessageBox, QFileDialog, QProgressDialog, QStyle, QSplitter)
...
class DbManagerDialog(QDialog):
    def __init__(self, role, parent=None):
        ...
        self._navigate_on_load_item = None # Aggiunto per la navigazione
        self.setup_ui()
        self.load_customers_table()

    def navigate_on_load(self, item_data):
        """Memorizza l'elemento da selezionare dopo il caricamento."""
        self._navigate_on_load_item = item_data

    def setup_ui(self):
        main_layout = QVBoxLayout(self)

        # 1. Le azioni in alto rimangono invariate
        ...
        self.customer_search_box.setPlaceholderText("CERCA CLIENTE...")
        self.customer_table = QTableWidget(0, 3)
        self.customer_table.setHorizontalHeaderLabels(["ID", "NOME", "INDIRIZZO"])
        ...
        self.destination_search_box.setPlaceholderText("CERCA DESTINAZIONE...")
        ...
        dest_layout.addWidget(self.destination_search_box)
        dest_layout.addWidget(self.destination_table)
        ...
        self.device_search_box.setPlaceholderText("CERCA DISPOSITIVO...")
        self.device_table = QTableWidget(0, 11)
        self.device_table.setHorizontalHeaderLabels(["ID", "DESCRIZIONE", "REPARTO", "S/N", "COSTRUTTORE", "MODELLO", "INV. CLIENTE", "INV. AMS", "INT. VERIFICA (MESI)", "STATO", "ULTIMA VERIFICA"])
        ...
        self.verification_search_box.setPlaceholderText("CERCA PER DATA, TECNICO, CODICE...")
        ...
        self.setup_table_style(self.verifications_table, hide_id=False, stretch_last=False)
        verif_buttons_layout = self.create_verification_buttons()
        verif_layout.addWidget(self.verification_search_box)
        verif_layout.addWidget(self.verifications_table)
        verif_layout.addLayout(verif_buttons_layout)

        # 3. Utilizzo di QSplitter per un layout flessibile
        # Splitter principale che divide la UI in sinistra e destra
        main_splitter = QSplitter(Qt.Horizontal, self)

        # Splitter per la colonna di sinistra (Clienti e Destinazioni)
        left_splitter = QSplitter(Qt.Vertical, self)
        left_splitter.addWidget(customers_group)
        left_splitter.addWidget(self.destinations_group)
        left_splitter.setSizes([200, 300]) # Dimensioni iniziali indicative

        # Splitter per la colonna di destra (Dispositivi e Storico)
        right_splitter = QSplitter(Qt.Vertical, self)
        right_splitter.addWidget(self.devices_group)
        right_splitter.addWidget(self.verifications_group)
        right_splitter.setSizes([400, 200]) # Diamo più spazio ai dispositivi

        # Aggiungiamo i due splitter verticali a quello orizzontale
        main_splitter.addWidget(left_splitter)
        main_splitter.addWidget(right_splitter)
        
        # Impostiamo le proporzioni iniziali (es. 35% a sinistra, 65% a destra)
        main_splitter.setSizes([self.width() * 0.35, self.width() * 0.65])

        # Aggiungiamo lo splitter principale al layout della finestra
        main_layout.addWidget(main_splitter)
        self.setLayout(main_layout)

        # 4. Connessioni (come prima)
    ...
    def setup_table_style(self, table, hide_id=True, stretch_last=True):
        """Imposta lo stile di base per le tabelle dell'anagrafica."""
        ...
        table.setStyleSheet("QTableWidget::item { vertical-align: middle; }")
    ...
    def create_top_actions(self):
        ...
        layout.addStretch()
        layout.addWidget(self.create_button("Genera Report per Periodo...", self.generate_monthly_reports, QStyle.SP_FileDialogToParent))
    ...
    def export_destination_table(self):
        ...
        # Utilizza la tua dialog per selezionare l'intervallo
        date_dialog = SingleCalendarRangeDialog(self)
        if date_dialog.exec() == QDialog.Accepted:
            # --- INIZIO CORREZIONE ---
            # Recupera le date dal dialogo. Assumiamo che restituisca già delle stringhe
            # o degli oggetti QDate. Il codice ora gestisce entrambi i casi.
            ...
            # Converte le date in stringhe, solo se sono oggetti QDate
            # Se sono già stringhe, le usa direttamente.
            if hasattr(start_date_obj, 'toString'):
                start_date = start_date_obj.toString("yyyy-MM-dd")
            else:
                start_date = str(start_date_obj)

            if hasattr(end_date_obj, 'toString'):
            ...
            self.thread = QThread()
            # Passa le stringhe delle date al worker
            self.worker = TableExportWorker(dest_id, output_path, start_date, end_date) # Already correct, but good to double check
    ...
    def reset_views(self, level='customer'):
        if level == 'customer': self.destination_table.setRowCount(0); self.destinations_group.setTitle("DESTINAZIONI / SEDI"); self.set_destination_buttons_enabled(False, False)
    ...
    def load_customers_table(self):
        ...
        for cust in customers:
            ...
            self.customer_table.setItem(row, 1, QTableWidgetItem(cust['name'].upper()))
            self.customer_table.setItem(row, 2, QTableWidgetItem(cust['address'].upper()))
    ...
    def customer_selected(self):
        ...
        if cust_id:
            ...
            self.destinations_group.setTitle(f"DESTINAZIONI PER '{customer_name.upper()}'")
            self.load_destinations_table(cust_id); self.set_destination_buttons_enabled(True, False)
    ...
    def destination_selected(self):
        ...
        if dest_id:
            ...
            self.devices_group.setTitle(f"DISPOSITIVI IN '{dest_name.upper()}'")
    ...
    def load_devices_table(self, destination_id):
        ...
        # CAMBIO 1: Chiamiamo la nuova funzione che prende TUTTI i dispositivi con i dati di verifica
        # Nota: questa funzione non filtra per destinazione, lo facciamo noi dopo
        all_devices = database.get_devices_with_last_verification()
        
        # Filtriamo i dispositivi per la destinazione corrente e per il testo di ricerca
        devices_to_show = [
            dev for dev in all_devices 
            if dev.get('destination_id') == destination_id and (
                not search_text or 
                search_text.lower() in str(dev.get('description', '')).lower() or
                search_text.lower() in str(dev.get('serial_number', '')).lower()
            )
        ]

        # Colori per le righe
        color_pass = QColor("#d4edda")  # Verde chiaro
        color_fail = QColor("#f8d7da")  # Rosso chiaro
        ...
        for dev in devices_to_show:
            ...
            # Popolamento delle celle (come prima)
            ...
            # NUOVA LOGICA: Popolamento colonna "Ultima Verifica"
            ...
            # NUOVA LOGICA: Colorazione della riga
            target_color = None
            last_outcome = dev.get('last_verification_outcome')
            if last_outcome == "PASSATO": # O 'CONFORME' se usi quello
                target_color = color_pass
            elif last_outcome == "FALLITO": # O 'NON CONFORME'
                target_color = color_fail
            
            if target_color:
                for col_index in range(self.device_table.columnCount()):
                    self.device_table.item(row, col_index).setBackground(target_color)
    ...
    def device_selected(self):
        ...
        if dev_id:
            ...
            self.verifications_group.setTitle(f"STORICO VERIFICHE PER '{dev_desc.upper()}'")
    ...
    def add_customer(self):
        ...
        if dialog.exec():
            ...
            except ValueError as e: QMessageBox.warning(self, "DATI NON VALIDI", str(e).upper())
    ...
    def delete_customer(self):
        ...
        if reply == QMessageBox.Yes:
            ...
            else: QMessageBox.critical(self, "ERRORE", message.upper())
    ...
    def delete_destination(self):
        ...
        if reply == QMessageBox.Yes:
            ...
            except ValueError as e: QMessageBox.critical(self, "ERRORE", str(e).upper())
    ...
    def move_device(self):
        ...
        if dialog.exec():
            ...
            if new_dest_id and new_dest_id != old_dest_id:
                ...
                except Exception as e: QMessageBox.critical(self, "ERRORE", f"IMPOSSIBILE SPOSTARE IL DISPOSITIVO: {str(e).upper()}")
    ...
    def on_export_finished(self, status, message):
        self.setWindowTitle("MANAGER ANAGRAFICHE")
        if status == "Success": QMessageBox.information(self, "ESPORTAZIONE COMPLETATA", message.upper())
        else: QMessageBox.warning(self, "ESPORTAZIONE", message.upper())

    def on_export_error(self, error_message):
        self.setWindowTitle("MANAGER ANAGRAFICHE"); QMessageBox.critical(self, "ERRORE ESPORTAZIONE", error_message.upper())
    ...
    def open_period_filter_dialog(self):
        ...
        except Exception as e: QMessageBox.critical(self, "ERRORE", f"IMPOSSIBILE RECUPERARE LO STATO: {str(e).upper()}")
    ...
    def generate_old_report(self):
        ...
        except Exception as e: QMessageBox.critical(self, "ERRORE", f"IMPOSSIBILE GENERARE IL REPORT: {str(e).upper()}")
    ...
    def show_all_customer_devices(self):
        ...
        customer_name = self.customer_table.item(self.customer_table.currentRow(), 1).text()
        self.devices_group.setTitle(f"TUTTI I DISPOSITIVI PER '{customer_name.upper()}'")
        ...
        for dev_row in devices:
            ...
            self.device_table.setItem(row, 0, QTableWidgetItem(str(dev.get('id')))); self.device_table.setItem(row, 1, QTableWidgetItem(str(dev.get('description')).upper())); self.device_table.setItem(row, 2, QTableWidgetItem(str(dev.get('department')).upper())); self.device_table.setItem(row, 3, QTableWidgetItem(str(dev.get('serial_number')).upper())); self.device_table.setItem(row, 4, QTableWidgetItem(str(dev.get('manufacturer')).upper())); self.device_table.setItem(row, 5, QTableWidgetItem(str(dev.get('model')).upper())); self.device_table.setItem(row, 6, QTableWidgetItem(str(dev.get('customer_inventory')).upper())); self.device_table.setItem(row, 7, QTableWidgetItem(str(dev.get('ams_inventory')).upper())); interval = dev.get('verification_interval'); interval_text = str(interval) if interval is not None else "N/A"; self.device_table.setItem(row, 8, QTableWidgetItem(interval_text.upper()))
            self.device_table.setItem(row, 9, QTableWidgetItem(status_text.upper()))
    ...
    def find_and_select_item(self, table: QTableWidget, item_id: int):
        """Cerca un ID nella prima colonna di una tabella e lo seleziona."""
