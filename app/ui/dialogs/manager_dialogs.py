...
from PySide6.QtWidgets import (
    ...
    QHeaderView, QAbstractItemView, QStyle, QMessageBox, QFileDialog, QProgressDialog
)
...
# --- INIZIO BLOCCO IMPORT DEL TUO PROGETTO ---
# Assicurati che questi import siano corretti per la struttura del tuo progetto.
# -------------------------------------------------------------------
...
import database
# ------------------------------------------------------------------
# --- FINE BLOCCO IMPORT DEL TUO PROGETTO ---
...
class DbManagerDialog(QDialog):
    ...
    def setup_customer_tab(self):
        ...
        self.tabs.addTab(self.customer_tab, "1. CLIENTI")
    ...
    def setup_destination_tab(self):
        ...
        self.tabs.addTab(self.destination_tab, "2. DESTINAZIONI / SEDI")
        layout = QVBoxLayout(self.destination_tab)
        self.destination_label = QLabel("SELEZIONA UN CLIENTE DALLA SCHEDA PRECEDENTE")
    ...
    def setup_device_tab(self):
        ...
        self.tabs.addTab(self.device_tab, "3. DISPOSITIVI")
        layout = QVBoxLayout(self.device_tab)
        self.device_label = QLabel("SELEZIONA UNA DESTINAZIONE DALLA SCHEDA PRECEDENTE")
    ...
    def setup_verification_tab(self):
        ...
        self.tabs.addTab(self.verification_tab, "4. STORICO VERIFICHE")
        layout = QVBoxLayout(self.verification_tab)
        self.verification_label = QLabel("SELEZIONA UN DISPOSITIVO DALLA SCHEDA PRECEDENTE")
    ...
    def create_customer_buttons(self):
        ...
        if self.user_role == 'technician':
            self.edit_cust_btn.setVisible(True)
            self.add_cust_btn.setVisible(True)
    ...
    def create_destination_buttons(self):
        ...
        if self.user_role == 'technician':
            self.edit_dest_btn.setVisible(True)
    ...
    def reset_views(self, level='customer'):
        if level == 'customer':
            ...
            self.destination_label.setText("SELEZIONA UN CLIENTE DALLA SCHEDA PRECEDENTE")
            self.set_destination_buttons_enabled(False, False)
        if level in ['customer', 'destination']:
            self.device_table.setRowCount(0)
            self.device_label.setText("SELEZIONA UNA DESTINAZIONE DALLA SCHEDA PRECEDENTE")
            self.set_device_buttons_enabled(False)
        if level in ['customer', 'destination', 'device']:
            self.verifications_table.setRowCount(0)
            self.verification_label.setText("SELEZIONA UN DISPOSITIVO DALLA SCHEDA PRECEDENTE")
