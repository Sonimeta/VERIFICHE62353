...
class MainWindow(QMainWindow):
    ...
    def handle_state_change(self, new_state: AppState):
        """Abilita/disabilita i componenti della UI in base allo stato."""
        is_idle = new_state == AppState.IDLE
        is_syncing = new_state == AppState.SYNCING

        # Abilita/disabilita l'intera finestra per prevenire qualsiasi interazione
        self.centralWidget().setEnabled(is_idle)
        self.menuBar().setEnabled(is_idle)

        # Fornisce un feedback visivo specifico per la sincronizzazione
        if is_syncing:
            self.statusBar().showMessage("Sincronizzazione in corso, attendere...", 0) # Messaggio persistente
            QApplication.setOverrideCursor(Qt.WaitCursor)
        else:
            self.statusBar().clearMessage()
            QApplication.restoreOverrideCursor()

        # Se lo stato è ERRORE, riabilita la UI ma mostra un messaggio di errore
        if new_state == AppState.ERROR:
            self.centralWidget().setEnabled(True)
            self.menuBar().setEnabled(True)
            self.statusBar().showMessage("Errore. Controllare i log.", 5000)
    # --- FINE MODIFICA ---
